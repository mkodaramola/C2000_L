...
void main(void)
{
    ...
    while (1) {
        ...
        // fallingEdgeDelta now holds time between two falling edges
        __asm(" NOP");
    }
}
...
__interrupt void ecap1ISR(void)
{
    ...
    if(c >= 9 && c <= 13) {
        ...
        if (period_ms >= 1.0f && period_ms <= 1.5f)data = (data << 1);
        else if(period_ms >= 2.0f && period_ms <= 2.5f) data = (data << 1) | 1;
